...
const withToolbarButton = createHigherOrderComponent(BlockEdit => {
  return props => {
    ...
    return (0,_wordpress_element__WEBPACK_IMPORTED_MODULE_1__.createElement)(Fragment, null, (0,_wordpress_element__WEBPACK_IMPORTED_MODULE_1__.createElement)(BlockControls, {
      ...
      icon: "editor-outdent",
      label: __('Add List', 'core-block-custom-attributes'),
      isActive: paragraphAttribute === 'custom',
      onClick: () => {
        if (paragraphAttribute === 'custom') {
          //var ele = document.getElementsByClassName("has-custom-attribute")[0];
          ...
          var index = wp.data.select('core/block-editor').getBlocks().map(function(block) { return block.clientId == selectionID; }).indexOf(true)+1 ;
         // wp.data.dispatch('core/block-editor').selectBlock(index);
          var selectionID1 = document.getSelection().focusNode.parentElement.id.split("block-")[1];
          wp.data.dispatch( 'core/block-editor' ).moveBlocksUp([selectionID1]);
          var selectionID2 = wp.data.select( 'core/block-editor' ).getAdjacentBlockClientId(selectionID);
          //console.log(wp.data.select( 'core/block-editor' ).getBlock(index));
          wp.data.dispatch( 'core/block-editor' ).removeBlock(selectionID2);
          ...
          setTimeout(function() {
           ...
           //var ele = document.getElementsByClassName("has-custom-attribute")[0];
            //ele.innerHTML += "<li>First</li><li>Second</li><li>Third</li>";
          var selectionID = document.getSelection().focusNode.parentElement.id.split("block-")[1];
